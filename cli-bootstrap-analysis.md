...
## 8. init / bootstrap / start 命令衔接说明

### 8.1 简洁总结

> **首次部署**：选择 `init` 或 `bootstrap` 完成数据库初始化 → `start`  
> **后续启动/升级**：`bootstrap`（执行新迁移）→ `start` 或直接 `start`

- `init` 和 `bootstrap` **不是互斥**的：`init` 完成后仍可执行 `bootstrap` 来执行版本升级的迁移
- `bootstrap` 是**幂等**的：
  - 数据库未安装时：执行完整初始化（建表 + 迁移 + 可选创建管理员）
  - 数据库已安装时：仅执行新的迁移
- `start` 要求数据库已安装 Directus

### 8.2 三个命令的核心行为对比

| 命令 | .env 依赖 | 输入来源 | 行为 |
|------|-----------|----------|------|
| `init` | ❌ 不依赖 | 交互式输入（inquirer） | 选择驱动 → 安装驱动 → 连接数据库 → 建表 + 迁移 → 生成 `.env` → 创建管理员 |
| `bootstrap` | ✅ 依赖 | `useEnv()`（所有环境变量） | 等待数据库 → `isInstalled()` 判断 → 未安装则完整初始化，已安装则仅迁移 |
| `start` | ✅ 依赖 | `useEnv()`（所有环境变量） | 验证数据库已安装 → 启动 API 服务器 |

### 8.3 场景一：开发者本地初始化（init → bootstrap → start）
...
directus init
   │
   ├─► 不依赖 .env，使用 createDBConnection() 直连
   │
   ▼
┌─────────────────────────────────────────────┐
│ 1. 选择数据库类型 → npm install 驱动         │
│ 2. 交互式输入数据库连接信息                   │
│ 3. runSeed() + runMigrations() → 数据库已安装 │
│ 4. 生成 .env 文件                            │
│ 5. 交互式创建管理员                          │
└─────────────────────────────────────────────┘
   │
...
当前状态：有 .env，数据库已安装
   │
   ▼
directus bootstrap（可选，用于版本升级）
   │
   ├─► 依赖 .env，isInstalled() 返回 true
   │
   ▼
┌─────────────────────────────────────────────┐
│ 仅执行新的 runMigrations()（幂等）           │
└─────────────────────────────────────────────┘
   │
   ▼
directus start → 服务运行
...
### 8.4 场景二：容器化自动化部署（bootstrap → start）

```
已配置 .env（或通过 CONFIG_PATH 指定配置文件）
   │
   ▼
directus bootstrap
   │
   ├─► 依赖 useEnv() 读取所有 DB_* 等配置
   │
   ▼
┌─────────────────────────────────────────────┐
│ 1. waitForDatabase() → 等待数据库就绪       │
│ 2. isInstalled() 判断                       │
│                                              │
│   ├─ 未安装（首次启动）：                     │
│   │    - installDatabase() → 建表            │
...
**1. init 和 bootstrap 不是互斥的**

`init` 执行完成后，数据库已处于"已安装"状态。此时执行 `bootstrap`，由于 `isInstalled()` 返回 `true`，会走到 `else` 分支：

```typescript
// bootstrap/index.ts:27-61
if ((await isInstalled()) === false) {
    // 首次初始化逻辑
} else {
    logger.info('Database already initialized, skipping install');
    logger.info('Running migrations...');
    await runMigrations(database, 'latest');  // 仅执行迁移
}
```

因此 `bootstrap` 可以在 `init` 之后用于：
- 执行 Directus 版本升级带来的新迁移
- 确保数据库状态与代码版本一致

**2. bootstrap 的输入来源**

`bootstrap` 通过 `useEnv()` 读取配置，输入来源包括：
- `.env` 文件
- `CONFIG_PATH` 指定的配置文件（`.js`, `.json`, `.yml` 等）
- 进程环境变量 `process.env`
- （注意优先级：配置文件 > 进程环境变量 > 默认值）

其中 `ADMIN_EMAIL` / `ADMIN_PASSWORD` 是**可选**的：

```typescript
// create-admin.ts:49-52
const adminEmail = admin?.email ?? env['ADMIN_EMAIL'];
const adminPassword = admin?.password ?? env['ADMIN_PASSWORD'];
if (!adminEmail || !adminPassword) return;  // 没有则不创建管理员
```

如果未配置这两个变量，`bootstrap` 首次启动时只会建表和执行迁移，不会创建管理员账户。
